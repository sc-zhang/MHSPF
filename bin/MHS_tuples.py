...
def get_opts():
    ...
    group.add_argument('-f', '--freq', help="Input frequence file", required=True)
    ...
    group.add_argument('-s', '--mhss_dis', help="Maximum distance between two MHSs, default=50", type=int, default=50)
...
def get_mhss(freq_file:str, lbound: int, ubound: int, max_mhss_dis: int, mhs_file:str, out_file: str):
    ...
    with open(out_file, 'w') as fout:
        for mhss in all_mhss[:sel_mhss]:
            ...
            print("\tPID: %d, [%d/%d] Dealing %s with (%s)"%(os.getpid(), sel_idx, sel_mhss, mhs_file, ','.join(pmhss_list)))
            tmp_mhss = []
